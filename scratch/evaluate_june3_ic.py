...
def main():
    ...
    target_time = pd.to_datetime("2026-06-03 20:00:00")
    ...
    # Let's print details of subsequent candles to see how mitigation occurred
    print("\nSubsequent H4 candles:")
    for j in range(idx + 1, min(len(df), idx + 6)):
        time_j = df['time'].iloc[j]
        high_j = df['High'].iloc[j]
        low_j = df['Low'].iloc[j]
        close_j = df['Close'].iloc[j]
        print(f"  {time_j}: Open={df['Open'].iloc[j]:.3f}, High={high_j:.3f}, Low={low_j:.3f}, Close={close_j:.3f}")
