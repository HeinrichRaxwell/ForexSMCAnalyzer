...
- `forward_exit_summary.csv`: closed-trade exit categories and net PnL by entry type, timeframe, and strategy.
- `strategy_policy_report.csv`: evidence-based deployment status for the reported groups.
...
The maintainer workflow `scripts/publish_daily_reports.ps1` stages public report
artifacts, README, and report tooling. `.env`, account state, scanner state, raw
tick cache, and raw scratch data are excluded.
